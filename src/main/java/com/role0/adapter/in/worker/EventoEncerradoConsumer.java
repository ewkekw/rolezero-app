...
public class EventoEncerradoConsumer {
    ...
    public void processarExpiracao(String mensagemIdBaseString) {
        ...
        if (evento.isIncidenteReportado()) {
            // Congelamento Legal
            System.out.println("CONSUMER [WORM S3]: Flag de Incidente ATIVA. Redirecionando Expurgo. Conservando Logs do Chat para a Party " + eventoId);
            chatNotification.congelarChatLegalmente(eventoId);
        } else {
            // Deleção Física Baseada na Arquitetura Zero Knowledge
            System.out.println("CONSUMER [ZERO-KNOWLEDGE]: Excluindo definitivamente Chat e Rastros Físicos Efêmeros do Role " + eventoId);
        }
